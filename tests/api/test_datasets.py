...
def _create_trace(db_path: Path, *, case_id: str, trace_id: str = "tr_promote") -> None:
...
def test_draft_regression_case_from_trace(tmp_path: Path) -> None:
    db = tmp_path / "selfevals.sqlite"
    client = TestClient(build_app(db_path=str(db)))
    created = _create_inline(client)
    source_case = created["cases"][0]
    _create_trace(db, case_id=source_case["id"])

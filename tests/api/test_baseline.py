...
def _iteration(itr_id: str, *, primary: float, f1: dict[str, float | None]) -> IterationRecord:
...
def client(tmp_path: Path) -> TestClient:
    ...
    try:
        ...
        with storage.open(WS) as scope:
            ...
            scope.put_entity(_iteration(ITR_BASE, primary=0.8, f1={"a": 0.9}))
            scope.put_entity(_iteration(ITR_WORSE, primary=0.6, f1={"a": 0.6}))
    finally:
        storage.close()

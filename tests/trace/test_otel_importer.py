...
def test_messages_extracted_via_openinference_native_attrs() -> None:
    ...
    s = trace.spans[0]  # type: ignore[attr-defined]
    assert isinstance(s, LLMCallSpan)
    msgs_in = s.provider_metadata["bootstrap.messages_in"]
    assert msgs_in == [
        {"role": "system", "content": "You are helpful."},
        {"role": "user", "content": "Hola"},
    ]
    msgs_out = s.provider_metadata["bootstrap.messages_out"]
...
def test_messages_extracted_via_gen_ai_alias_attrs() -> None:
    ...
    s = trace.spans[0]  # type: ignore[attr-defined]
    assert isinstance(s, LLMCallSpan)
    assert s.provider_metadata["bootstrap.messages_in"] == [
        {"role": "user", "content": "What is 2+2?"}
    ]
    assert s.provider_metadata["bootstrap.messages_out"] == [
...
def test_message_index_order_is_numeric_not_lexical() -> None:
    ...
    s = trace.spans[0]  # type: ignore[attr-defined]
    contents = [m["content"] for m in s.provider_metadata["bootstrap.messages_in"]]
...
def test_openinference_native_wins_when_both_families_present() -> None:
    ...
    s = trace.spans[0]  # type: ignore[attr-defined]
    assert s.provider_metadata["bootstrap.messages_in"] == [{"role": "user", "content": "native"}]
...
def test_no_messages_leaves_hashes_none() -> None:
    ...
    assert "bootstrap.messages_in" not in s.provider_metadata
    assert "bootstrap.messages_out" not in s.provider_metadata

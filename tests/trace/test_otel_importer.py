...
def test_parent_child_preserved() -> None:
    trace = _imp([
        {"span_id": "sp_a", "name": "turn", "start_time": T0, "end_time": T1,
         "attributes": {"openinference.span.kind": "AGENT"}},
        {"span_id": "sp_b", "parent_span_id": "sp_a", "name": "model",
...
def test_epoch_seconds_and_nanos_both_parsed() -> None:
    trace = _imp([
        {"span_id": "sp_a", "name": "x", "start_time": 1716115200,
         "duration_ms": 1500,
         "attributes": {"foo": "bar"}},
        {"span_id": "sp_b", "name": "x", "start_time": 1716115200000000000,
         "duration_ms": 700,
         "attributes": {"foo": "bar"}},
    ])
    spans = trace.spans  # type: ignore[attr-defined]
...
def test_invalid_workspace_rejected() -> None:
    with pytest.raises(ValueError):
        import_otel_spans(
            [], workspace_id="", run=RunInfo(run_id="r"),
            agent=AgentSnapshotRef(agent_id="ag_x", agent_version=1),
        )

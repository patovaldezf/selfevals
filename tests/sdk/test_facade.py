"""Unit tests for selfevals.sdk.facade."""
...
def test_init_raises_on_different_project() -> None:
    ...
    with pytest.raises(facade.SelfEvalsAlreadyInitialized):
...
def test_endpoint_resolution_explicit_wins(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("SELFEVALS_OTLP_ENDPOINT", "http://from-env:1234")
...
def test_endpoint_resolution_selfevalss_env_wins_over_otel(
...
def test_endpoint_resolution_otel_fallback(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("SELFEVALS_OTLP_ENDPOINT", raising=False)
    monkeypatch.setenv("OTEL_EXPORTER_OTLP_ENDPOINT", "http://otel-env:9999")
    result = facade.init(project="p", instrument=[])
...
def test_top_level_init_and_shutdown_proxies() -> None:
    ...
    result = selfevals.init(project="proj1", instrument=[])
    assert result.project == "proj1"
    assert selfevals.is_initialized()
    selfevals.shutdown()
    assert not selfevals.is_initialized()

"""selfevals.init() — the one-line user-side activation.
...
from selfevals.sdk import context as ctx
from selfevals.sdk.auto_instrument import InstallReport, install_instrumentors
from selfevals.sdk.exporter import build_exporter

logger = logging.getLogger("selfevals.sdk")


class SelfEvalsAlreadyInitialized(RuntimeError):  # noqa: N818 — public API name
...
def init(
    *,
    ...
    """Activate selfevals telemetry. See module docstring for behaviour."""
    ...
    if _state is not None:
        if _state.project != project:
            raise SelfEvalsAlreadyInitialized(
                f"selfevals.init() already called with project={_state.project!r}; "
    ...
    # Try the heavy OTel SDK imports. If the user installed selfevals
    ...
    try:
        ...
        result = InitResult(
            ...
            endpoint=None,
            warnings=[
                "OpenTelemetry SDK not installed; selfevals.init() is a no-op. "
    ...
    resource = Resource.create(
        {
            ...
            "selfevals.project": project,
            "selfevals.iteration_id": (current.iteration_id if current else "") or "",
            "selfevals.run_id": (current.run_id if current else "") or "",
        }
    )
    ...
    if propagate_to_parent and _is_real_tracer_provider(existing_provider):
        ...
        owns_provider = False
        warnings = [
            "An existing TracerProvider was detected; selfevals is attaching "
    ...
    if resolved_endpoint is None:
        warnings.append(
            "No OTLP endpoint configured (set SELFEVALS_OTLP_ENDPOINT or pass endpoint=). "
...
def _resolve_endpoint(explicit: str | None) -> str | None:
    if explicit is not None:
        return explicit
    env_endpoint = os.environ.get("SELFEVALS_OTLP_ENDPOINT")
    if env_endpoint:
        return env_endpoint

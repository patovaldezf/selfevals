"""OpenTelemetry → selfevals Trace importer.
...
- llm.input_messages.{i}.message.*    ┘     selfevals.messages_in metadata)
- gen_ai.completion.{i}.role/content  ┐  → output messages (output.content_hash
- llm.output_messages.{i}.message.*   ┘     + selfevals.messages_out metadata)
...
(`selfevals.messages_in` / `selfevals.messages_out`). When these spans are
...
def _build_llm_span(span: dict[str, Any], attrs: dict[str, Any]) -> LLMCallSpan:
    ...
    if messages_in:
        provider_metadata["selfevals.messages_in"] = messages_in
    if messages_out:
        provider_metadata["selfevals.messages_out"] = messages_out
...
def import_otel_spans(
    ...
    *,
    ...
    """Build a selfevals Trace from a flat list of OTel-style span dicts."""
    if not workspace_id:

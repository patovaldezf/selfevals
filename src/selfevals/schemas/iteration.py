...
from selfevals.schemas.experiment import EditableContract, Experiment
...
class ProposerInputs(SelfEvalsModel):
...
class ExecutionInfo(SelfEvalsModel):
...
class MetricObservation(SelfEvalsModel):
    name: NonEmptyStr
    value: float
    delta_vs_baseline: float | None = None


class IterationMetrics(SelfEvalsModel):
...
class HumanRationale(SelfEvalsModel):
...
class DecisionRationale(SelfEvalsModel):
    automated: NonEmptyStr
    human: HumanRationale | None = None


class NextAction(SelfEvalsModel):
    kind: NonEmptyStr

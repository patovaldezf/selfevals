"""bootstrap.init() — the one-line user-side activation.
...
from bootstrap.sdk import context as ctx
from bootstrap.sdk.auto_instrument import InstallReport, install_instrumentors
from bootstrap.sdk.exporter import build_exporter

logger = logging.getLogger("bootstrap.sdk")


class BootstrapAlreadyInitialized(RuntimeError):  # noqa: N818 — public API name
...
def init(
    *,
    ...
    """Activate bootstrap telemetry. See module docstring for behaviour."""
    ...
    if _state is not None:
        if _state.project != project:
            raise BootstrapAlreadyInitialized(
                f"bootstrap.init() already called with project={_state.project!r}; "
    ...
    # Try the heavy OTel SDK imports. If the user installed bootstrap
    ...
    try:
        ...
        result = InitResult(
            ...
            endpoint=None,
            warnings=[
                "OpenTelemetry SDK not installed; bootstrap.init() is a no-op. "
                "Install with `pip install 'bootstrap[telemetry]'`."
            ],
    ...
    resource = Resource.create(
        {
            ...
            "bootstrap.iteration_id": (current.iteration_id if current else "") or "",
            "bootstrap.run_id": (current.run_id if current else "") or "",
        }
    )
    ...
    if propagate_to_parent and _is_real_tracer_provider(existing_provider):
        ...
        owns_provider = False
        warnings = [
            "An existing TracerProvider was detected; bootstrap is attaching "
    ...
    if resolved_endpoint is None:
        warnings.append(
            "No OTLP endpoint configured (set BOOTSTRAP_OTLP_ENDPOINT or pass endpoint=). "
...
def _resolve_endpoint(explicit: str | None) -> str | None:
    if explicit is not None:
        return explicit
    env_endpoint = os.environ.get("BOOTSTRAP_OTLP_ENDPOINT")
    if env_endpoint:
        return env_endpoint

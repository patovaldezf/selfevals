...
A Trace captures everything bootstrap needs to:
...
class EnvironmentInfo(BootstrapModel):
...
class FinalState(BootstrapModel):
    status: TraceState
    error: str | None = None


class ReasoningBlock(BootstrapModel):
...
class TokenBreakdown(BootstrapModel):
...
class CostBreakdown(BootstrapModel):
...
class ToolUseRequest(BootstrapModel):
    tool: NonEmptyStr
    tool_use_id: NonEmptyStr


class LLMOutput(BootstrapModel):
...
class RetrievedDoc(BootstrapModel):
    doc_id: NonEmptyStr
...
class TraceLink(BootstrapModel):
    kind: Literal["paraphrase_variant", "replay_of", "spawned_by"]
    trace_id: NonEmptyStr


class TraceMetrics(BootstrapModel):
...
# --- spans (discriminated union on `kind`) -----------------------------------


class _SpanBase(BootstrapModel):
    id: NonEmptyStr
...
class GraderResult(BootstrapModel):
    grader: NonEmptyStr
    label: str

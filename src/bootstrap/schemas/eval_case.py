...
from bootstrap.schemas.registry import RiskProfile


class FeatureTag(BootstrapModel):
...
class SourceInfo(BootstrapModel):
...
class GroundTruthSpec(BootstrapModel):
...
class CaseTaxonomy(BootstrapModel):
    level: Level
    feature: FeatureTag
    source: SourceInfo
...
class CaseMetadata(BootstrapModel):

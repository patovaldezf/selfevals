...
class MetricTarget(BootstrapModel):
...
class TargetSpec(BootstrapModel):
...
class EditableContract(BootstrapModel):
...
class SearchSpace(BootstrapModel):
...
class FrozenSnapshot(BootstrapModel):
...
class ProposerSpec(BootstrapModel):
...
class CounterfactualSpec(BootstrapModel):
    enabled: bool = False
...
class JudgeDefenses(BootstrapModel):
...
class ReliabilitySpec(BootstrapModel):
    repetitions_per_case: int = Field(default=1, ge=1, le=100)
...
class DecisionPolicy(BootstrapModel):

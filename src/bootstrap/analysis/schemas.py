...
These are the contract between bootstrap and an external coding agent (the
`error-analysis` skill). bootstrap emits an `AnalysisBundle` (pull) and ingests
an `AnalysisResult` (push). Get these right and any agent can honour the
protocol. See docs/spec/error_analysis_design.md §4.

They are plain `BootstrapModel`s (not entities) — transport shapes, not stored
...
from bootstrap.schemas._base import BootstrapModel, NonEmptyStr

ANALYSIS_SCHEMA_VERSION = "1.0.0"


# --- pull: AnalysisBundle (bootstrap → agent) --------------------------------


class TaxonomyEntry(BootstrapModel):
...
class BundleGrade(BootstrapModel):
    label: str
...
class BundleMessage(BootstrapModel):
    role: str
    content: str


class BundleErrorSpan(BootstrapModel):
    kind: str
    name: str
    error: str | None = None


class BundleTrace(BootstrapModel):
...
class AnalysisBundle(BootstrapModel):
...
# --- push: AnalysisResult (agent → bootstrap) --------------------------------


class Assignment(BootstrapModel):
...
class ProposedMode(BootstrapModel):
...
class Hypothesis(BootstrapModel):
...
class AnalysisResult(BootstrapModel):
    schema_version: str = ANALYSIS_SCHEMA_VERSION

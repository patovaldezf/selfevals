"""Embedded OTLP/HTTP receiver hosted inside `bootstrap run`.
...
Optional publish hook: when running under `bootstrap serve`, the
embedded broker is installed via `set_publisher()` and every routed
span is fan-out'd to live SSE subscribers. `bootstrap run` (CLI-only)
...
from bootstrap.runner.otlp_to_recorder import (
    DecodedSpan,
...
class SpanPublisher:
    ...
    `bootstrap serve` installs a concrete one backed by `SpanBroker`."""
...
class ReceiverHandle:
    ...
    def set_publisher(self, publisher: SpanPublisher | None) -> None:
        ...
        routed span. Used by `bootstrap serve` to wire SSE; CLI-only
        runs leave it None."""
...
def _make_handler(ingest: _SpanIngest, stats: ReceiverStats) -> type[BaseHTTPRequestHandler]:
    class _Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:
            ...
            self.send_header("Connection", "close")
            self.end_headers()
            self.wfile.write(b"bootstrap-otlp-receiver")

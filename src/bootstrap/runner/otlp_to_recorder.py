"""Bridge OTLP protobuf payloads → bootstrap Trace spans.
...
`bootstrap.trace.otel_importer` already consumes, then feeds those dicts
    ...
    for resource_spans in request.resource_spans:
        ...
        for scope_spans in resource_spans.scope_spans:
            for span in scope_spans.spans:
                ...
                # Resource attributes (like bootstrap.iteration_id) get
...
def feed_to_recorder(spans: list[DecodedSpan], recorder: Any) -> int:
    ...
    if not spans:
        return 0
    from bootstrap.schemas.enums import SpanKind
    from bootstrap.schemas.trace import LLMCallSpan, Span
    from bootstrap.trace.otel_importer import (
        _build_agent_turn,
        _build_custom,
        _build_llm_span,

...
Applied versions are tracked in a `_selfeval_migrations` table.
...
def apply_migrations(conn: sqlite3.Connection) -> list[int]:
    ...
    conn.execute(
        "CREATE TABLE IF NOT EXISTS _selfeval_migrations ("
    ...
    applied = {
        row[0] for row in conn.execute("SELECT version FROM _selfeval_migrations").fetchall()
    }
    ...
    for migration in discover_migrations():
        ...
        conn.execute(
            "INSERT INTO _selfeval_migrations (version, name) VALUES (?, ?)",
...
def current_version(conn: sqlite3.Connection) -> int:
    ...
    row = conn.execute("SELECT MAX(version) FROM _selfeval_migrations").fetchone()

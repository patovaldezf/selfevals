...
from selfeval.schemas.registry import RiskProfile


class FeatureTag(SelfEvalModel):
...
class SourceInfo(SelfEvalModel):
...
class GroundTruthSpec(SelfEvalModel):
...
class CaseTaxonomy(SelfEvalModel):
    level: Level
    feature: FeatureTag
    source: SourceInfo
...
class CaseMetadata(SelfEvalModel):

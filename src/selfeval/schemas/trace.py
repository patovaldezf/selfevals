...
A Trace captures everything selfeval needs to:
...
class AgentSnapshotRef(SelfEvalModel):
...
class EnvironmentInfo(SelfEvalModel):
...
class FinalState(SelfEvalModel):
    status: TraceState
    error: str | None = None


class ReasoningBlock(SelfEvalModel):
...
class TokenBreakdown(SelfEvalModel):
...
class CostBreakdown(SelfEvalModel):
...
class ToolUseRequest(SelfEvalModel):
    tool: NonEmptyStr
    tool_use_id: NonEmptyStr


class LLMOutput(SelfEvalModel):
...
class RetrievedDoc(SelfEvalModel):
    doc_id: NonEmptyStr
...
class TraceLink(SelfEvalModel):
    kind: Literal["paraphrase_variant", "replay_of", "spawned_by"]
    trace_id: NonEmptyStr


class TraceMetrics(SelfEvalModel):
...
class GraderResult(SelfEvalModel):
    grader: NonEmptyStr
    label: str
...
class TraceOutputs(SelfEvalModel):

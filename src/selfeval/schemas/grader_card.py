...
class HumanReference(SelfEvalModel):
...
class CalibrationMetrics(SelfEvalModel):
...
class CalibrationThresholds(SelfEvalModel):
...
class DegradeBehavior(SelfEvalModel):

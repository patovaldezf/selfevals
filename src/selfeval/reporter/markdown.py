...
from selfeval.reporter._metrics import CostTimeSummary, compute_cost_time_summary
...
def _next_steps_lines(result: OptimizationResult) -> list[str]:
    ...
    out.append("")
    out.append("```bash")
    out.append(f"selfeval iteration list {ws_id} {exp_id}")
    out.append(f"selfeval experiment show {ws_id} {exp_id}")
    if len(result.iterations) >= 2:
        a = result.iterations[0].iteration_record.id
        b = result.iterations[-1].iteration_record.id
        out.append(f"selfeval compare {ws_id} {a} {b}")
    out.append(f"selfeval report {ws_id} {exp_id} --format json")
    out.append("```")
    return out

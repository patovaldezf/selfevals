"""OpenTelemetry → selfeval Trace importer.
...
- llm.input_messages.{i}.message.*    ┘     selfeval.messages_in metadata)
- gen_ai.completion.{i}.role/content  ┐  → output messages (output.content_hash
- llm.output_messages.{i}.message.*   ┘     + selfeval.messages_out metadata)
...
(`selfeval.messages_in` / `selfeval.messages_out`). When these spans are
...
from selfeval._internal.hashing import content_hash
...
def _build_llm_span(span: dict[str, Any], attrs: dict[str, Any]) -> LLMCallSpan:
    ...
    if messages_in:
        provider_metadata["selfeval.messages_in"] = messages_in
    if messages_out:
        provider_metadata["selfeval.messages_out"] = messages_out
...
def import_otel_spans(
    ...
    *,
    ...
    """Build a selfeval Trace from a flat list of OTel-style span dicts."""
    if not workspace_id:
    ...
    # can be reassembled into their thread.

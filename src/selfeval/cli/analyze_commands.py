...
from selfeval.analysis.schemas import AnalysisResult
from selfeval.cli.commands import _require_entity, _storage
...
def cmd_analyze_push(args: argparse.Namespace) -> int:
    raw = sys.stdin.read()
    if not raw.strip():
        raise SelfEvalUserError("analyze push expects an AnalysisResult JSON on stdin")
    try:
        result = AnalysisResult.model_validate_json(raw)
    except ValueError as exc:
        raise SelfEvalUserError(f"invalid AnalysisResult JSON: {exc}") from exc
    ...
    try:
        ...
        raise SelfEvalUserError(str(exc)) from exc
    finally:
        storage.close()
    ...
    if summary.created_candidates:
        print("\nnew candidates (promote with `selfeval failuremode promote <id>`):")
...
def cmd_failuremode_merge(args: argparse.Namespace) -> int:
    ...
    if src.id == dst.id:
        raise SelfEvalUserError("cannot merge a mode into itself")
...
def cmd_failuremode_edit(args: argparse.Namespace) -> int:
    ...
    if args.title is None and args.definition is None:
        raise SelfEvalUserError("nothing to edit: pass --title and/or --definition")

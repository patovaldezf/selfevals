...
* If the failure is something a user can fix by changing inputs,
  ...
  :class:`SelfEvalUserError` with a tight, file-relative message and
...
from selfeval.repo.loader import LoaderError, load_experiment_spec
from selfeval.runner.adapters import AdapterError

if False:  # for type checkers only, no runtime cycle.
    from selfeval.repo.loader import ExperimentSpec
...
def load_spec(path: str | Path, *, workspace_id: str | None = None) -> ExperimentSpec:
    ...
    Wraps :func:`selfeval.repo.loader.load_experiment_spec`. Catches
    raw YAML parser errors and `LoaderError` and re-raises them as
    :class:`SelfEvalUserError` so the CLI prints a clean single line.
    ...
    try:
        ...
        if dataset is not None:
            ...
            raise err from exc
        raise SelfEvalUserError(str(exc), hint=_yaml_hint_if_relevant(spec_path, exc)) from exc
    except yaml.YAMLError as exc:  # pragma: no cover - loader already wraps this
        raise SelfEvalUserError(
        ...
        raise SelfEvalUserError(f"experiment spec not found: {spec_path}") from exc


def dataset_not_found(path: Path) -> SelfEvalUserError:
    ...
    return SelfEvalUserError(f"dataset path {str(path)!r} not found", hint=hint)


def unknown_grader(name: str, available: list[str]) -> SelfEvalUserError:
    ...
    return SelfEvalUserError(
        f"grader {name!r} not registered; available: {available_str}",
        hint=hint,
    )


def wrap_adapter_error(exc: Exception, *, url: str | None = None) -> SelfEvalUserError:
    ...
    return SelfEvalUserError(f"adapter error{target}: {exc}")


def wrap_sqlite_error(exc: sqlite3.Error, *, db_path: Path | str) -> SelfEvalUserError:
    ...
    if "malformed" in msg or "corrupt" in msg or "not a database" in msg:
        return SelfEvalUserError(
            f"sqlite database {db_path} is corrupted or not a valid selfeval db",
            hint="back up the file and re-run with `--db <new-path>` to start clean",
        )
    return SelfEvalUserError(f"sqlite error at {db_path}: {exc}")

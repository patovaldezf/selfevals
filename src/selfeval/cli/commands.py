...
from selfeval.cli import _friendly
from selfeval.decision.matrix import DecisionMatrixEvaluator
from selfeval.graders.base import Grader
...
from selfeval.graders.registry import (
    available_graders,
    register_grader,
    resolve_graders,
    unregister_grader,
...
from selfeval.schemas.enums import ProposerStrategy
...
from selfeval.storage.seed import seed_failure_taxonomy, seed_workspace
from selfeval.storage.sqlite import SQLiteStorage

if TYPE_CHECKING:
    from selfeval.schemas._base import BaseEntity


class CommandError(SelfEvalUserError):
    """Raised for user-correctable errors. CLI prints and exits 2.

    Thin alias of :class:`selfeval._errors.SelfEvalUserError` so the
    rest of this module keeps the historical name. Anything new outside
    the CLI should raise :class:`SelfEvalUserError` directly.
    """
...
def cmd_examples_copy(args: argparse.Namespace) -> int:
    ...
    print("")
    print("Run:")
    print(f"  selfeval run {target_root / 'evals' / 'experiments' / 'example_pingpong.yaml'} --no-persist")
    return 0


def _copy_example_tree(*, name: str, target_root: Path) -> list[Path]:
    source_root = resources.files("selfeval.examples").joinpath("evals")
    files = {
...
def _reconstruct_result(
    ...
    """
    from selfeval.schemas.iteration import Proposal
...
def _ensure_cwd_on_path() -> None:
    ...
    `uv run selfeval ...` invokes a console script whose `sys.path` does
...
def _register_grader_specs(spec: ExperimentSpec) -> list[str]:
    ...
    across consecutive invocations of `selfeval run`.
...
def _resolve_case_graders(cases: Sequence[object]) -> list[Grader]:
    ...
    # `resolve_graders` raises SelfEvalUserError if any name is unknown,

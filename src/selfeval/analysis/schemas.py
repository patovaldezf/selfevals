...
These are the contract between selfeval and an external coding agent (the
`error-analysis` skill). selfeval emits an `AnalysisBundle` (pull) and ingests
an `AnalysisResult` (push). Get these right and any agent can honour the
protocol. See docs/spec/error_analysis_design.md §4.

They are plain `SelfEvalModel`s (not entities) — transport shapes, not stored
...
from selfeval.schemas._base import NonEmptyStr, SelfEvalModel

ANALYSIS_SCHEMA_VERSION = "1.0.0"


class TaxonomyEntry(SelfEvalModel):
...
class BundleGrade(SelfEvalModel):
    label: str
...
class BundleMessage(SelfEvalModel):
    role: str
    content: str


class BundleErrorSpan(SelfEvalModel):
    kind: str
    name: str
    error: str | None = None


class BundleTrace(SelfEvalModel):
...
class AnalysisBundle(SelfEvalModel):
...
class Assignment(SelfEvalModel):
...
class ProposedMode(SelfEvalModel):
...
class Hypothesis(SelfEvalModel):
...
class AnalysisResult(SelfEvalModel):
    schema_version: str = ANALYSIS_SCHEMA_VERSION

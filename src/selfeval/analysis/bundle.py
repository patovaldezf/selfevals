...
from selfeval.schemas.trace import ErrorSpan, LLMCallSpan, ToolCallSpan, Trace
...
def _transcript(trace: Trace) -> list[BundleMessage]:
    ...
    (`selfeval.messages_in` / `selfeval.messages_out`) on each LLM span.
    ...
    for span in trace.spans:
        if not isinstance(span, LLMCallSpan):
            continue
        for key in ("selfeval.messages_in", "selfeval.messages_out"):

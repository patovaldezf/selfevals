...
registered raises a :class:`SelfEvalUserError` with a list of the
...
def resolve_graders(names: list[str]) -> list[Grader]:
    ...
    Raises :class:`SelfEvalUserError` with the list of available names
    when any entry is missing — this is the path that produces the
    "Grader 'foo' not registered" message in the troubleshooting doc.
    """
    from selfeval.cli._friendly import unknown_grader  # avoid CLI ↔ graders import cycle
...
# imports `selfeval.graders` (or just this module) sees the defaults.

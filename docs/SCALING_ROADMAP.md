...
| --- | ----------------------------------------------------------------------- | -------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------- | -------- | --------- |
| ★1  | **Confusion failure-mode** (exact-match → `misclassified:X->Y`)         | exact-match es callejón sin salida diagnóstico                 | ❌ `detail="expected != actual"` genérico (`deterministic.py:213`)                                          | S        | SF-1      |
| 2   | **Errored/timeout fuera de pass@1**                                     | hoy un agente colgado cuenta como 0.0 de calidad               | 🟡 `GradeLabel.ERROR` existe (`base.py:25`) pero se colapsa a 0.0, no se excluye                            | S        | SF-1      |
| 3   | **`consistency_rate` al reporte**                                       | el no-determinismo se computa pero no se ve                    | 🟡 computado (`aggregator.py:74`), no renderizado (`markdown.py`)                                           | S        | SF-1      |

...
Locked-in choices for the bootstrap web surface. Deviations from the
...
Per the prompt. A stub `X-Bootstrap-User: local` header travels
...
## Backend service: FastAPI under `src/bootstrap/api/`

Per recommendation. New console script `bootstrap-api`. Reuses the
